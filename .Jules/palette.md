...
## 2024-05-24 - Accordion Expansion Accessibility
**Learning:** Collapsible sections like `Accordion` components are often built using generic `TouchableOpacity` elements. Without explicitly passing `accessibilityState={{ expanded: boolean }}`, screen reader users have no way to know if activating the section will reveal or hide content, reducing navigability.
**Action:** Always ensure expandable interactive components explicitly define their `accessibilityRole="button"`, provide a descriptive `accessibilityLabel` (e.g., the section title), and dynamically manage their `accessibilityState` and `accessibilityHint` to reflect their expanded/collapsed status.
