...
## 2024-05-24 - Core Button Wrapper Accessibility
**Learning:** Core layout wrappers like `Button` and `RippleButton` often mask the interactive properties of their inner `TouchableOpacity` elements, hiding disabled and loading states from screen readers despite handling those props for visual styling.
**Action:** Always verify core reusable button wrappers explicitly map incoming disabled and loading states into an `accessibilityState={{ disabled: boolean, busy: boolean }}` object on their native interactive primitives.
