...
## 2026-03-02 - Transient Overlay (Toast) Accessibility
**Learning:** Adding `accessible={true}` to transient overlays (like Toasts) that contain buttons causes screen readers to group all children, hiding the internal interactive elements from the focus tree.
**Action:** Use `accessibilityLiveRegion` (polite or assertive) and `accessibilityRole="alert"` on the main container instead of `accessible={true}`, and ensure any internal action or dismiss buttons have explicit `accessibilityRole="button"` and `accessibilityLabel`.
