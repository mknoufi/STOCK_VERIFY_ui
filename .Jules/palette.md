...
## 2024-05-24 - Transient State Icon Buttons
**Learning:** Icon-only buttons with transient states (like refreshing or saving) require explicit labeling to be audible to screen readers. Crucially, their "busy" state must be conveyed via `accessibilityState={{ disabled: loading, busy: loading }}` so users know when an asynchronous action is actively occurring.
**Action:** Always verify custom icon-only components that handle async states expose dynamic `accessibilityState` props.
