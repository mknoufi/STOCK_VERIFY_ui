...
## 2026-09-06 - LogoutButton accessibility\n**Learning:** In React Native, returning a separate UI element during a loading state completely removes the original element from the accessibility tree, breaking focus for screen reader users mid-action.\n**Action:** Always maintain a single `TouchableOpacity` wrapper for actionable components. Handle loading states by using `accessibilityState={{ disabled: loading, busy: loading }}`, applying `opacity: 0` to the core content to preserve layout, and absolutely positioning an `ActivityIndicator` over it.
