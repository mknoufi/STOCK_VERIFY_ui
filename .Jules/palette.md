...
## 2026-03-01 - Expandable Section Accessibility
**Learning:** Custom expandable components built with `TouchableOpacity` like Accordions must explicitly define `accessibilityRole="button"` and `accessibilityState={{ expanded: boolean }}` to properly announce their interactive state and toggling action to screen readers.
**Action:** Always verify custom expandable sections or accordions include `accessibilityRole="button"`, an `accessibilityLabel`, an `accessibilityHint` indicating the toggle action, and the `expanded` state.
