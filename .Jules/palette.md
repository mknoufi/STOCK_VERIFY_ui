...
## 2026-03-01 - Custom Action Button Accessibility
**Learning:** Custom UI components built over `TouchableOpacity` like `Button` or `RippleButton` often lack default accessibility behaviors like role or states (e.g., `busy` during loading). When developers implement features that conditionally render loaders, screen readers don't announce these states without explicit properties.
**Action:** Always ensure that custom button components explicitly pass down `accessibilityRole="button"`, dynamic `accessibilityLabel`, `accessibilityHint`, and crucial `accessibilityState` details like `disabled` and `busy` (when loading) to enable proper screen reader compatibility.
