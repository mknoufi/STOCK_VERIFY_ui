...
## 2024-05-24 - Preserving screen reader focus during button loading states
**Learning:** In React Native, returning a completely separate loading component (or a disabled version) inside a button component when loading starts removes the original element from the accessibility tree, breaking focus for screen reader users.
**Action:** To prevent layout jumps and preserve focus during loading states, maintain a single button wrapper, use `accessibilityState={{ busy: true, disabled: true }}`, keep the original content with `opacity: 0`, and absolutely position the `ActivityIndicator` over it.
