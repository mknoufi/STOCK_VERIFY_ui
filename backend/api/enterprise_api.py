...
async def add_to_ip_list(
    ...
    success = await security_service.add_to_ip_list(
        entry.ip_address, list_type, entry.reason, current_user.get("username"), expires_at
    )
...
async def unlock_account(
    body: UnlockAccountRequest, request: Request, current_user: dict = Depends(require_admin)
) -> Dict[str, Any]:
...
async def create_feature_flag(
    flag: FeatureFlagRequest, request: Request, current_user: dict = Depends(require_admin)
) -> Dict[str, Any]:
...
async def update_feature_flag(
    key: str, updates: Dict[str, Any], request: Request, current_user: dict = Depends(require_admin)
) -> Dict[str, Any]:
...
async def set_retention_policy(
    policy: RetentionPolicyRequest, request: Request, current_user: dict = Depends(require_admin)
) -> Dict[str, Any]:
...
async def create_data_request(
    body: DataSubjectRequestCreate, request: Request, current_user: dict = Depends(require_admin)
) -> Dict[str, Any]:
...
async def process_data_request(
    ...
    if req_details["request_type"] == "access":
        ...
        raise HTTPException(
            400, f"Request type '{req_details['request_type']}' requires manual processing"
        )
