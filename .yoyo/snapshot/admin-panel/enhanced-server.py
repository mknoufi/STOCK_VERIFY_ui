        ...
        for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
            try:
                ...
                if 'server.py' in cmdline and 'backend' in cmdline:
                    return {'running': True, 'pid': proc.info['pid'], 'port': 8000}
            except (psutil.NoSuchProcess, psutil.AccessDenied):
                continue
...
class EnhancedAdminHandler(http.server.SimpleHTTPRequestHandler):
        ...
        try:
            ...
            self.send_json_response({
                ...
                'data': {
                    ...
                    'security_score': max(0, 100 - (failed_logins * 5) - (suspicious_activity * 10)),
    ...
    def handle_logs(self, query_string):
        """Enhanced log handling with filtering"""
        try:
            ...
            self.send_json_response({
                ...
                'data': {
                    'logs': filtered_logs,
                    'total_count': len(metrics.system_logs),
    ...
    def handle_sql_test(self):
        ...
        try:
            ...
            if result == 0:
                ...
                metrics.add_security_event('sql_test', f'SQL Server connection test failed to {host}:{port}', 'warning')
                self.send_json_response({
                    'success': False,
    ...
    def handle_start(self):
        ...
        try:
            if script.exists():
                ...
                self.send_json_response({'success': False, 'message': 'Start script not found'}, 500)
        except Exception as e:
