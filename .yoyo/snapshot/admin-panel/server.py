...
class AdminPanelHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def check_frontend(self):
        ...
        try:
            for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                try:
                    cmdline = ' '.join(proc.info.get('cmdline', []))
                    if 'expo' in cmdline.lower() or 'metro' in cmdline.lower() or 'node' in cmdline.lower():
                        port = self.get_port_from_process(proc.info['pid'])
    ...
    def handle_mongodb_start(self):
        ...
        try:
            if script.exists():
                subprocess.Popen(['bash', str(script)], cwd=str(project_root))
                self.send_json_response({'success': True, 'message': 'MongoDB start command issued'})
            else:
                # Try direct start
                import os
            ...
            for proc in psutil.process_iter(['pid', 'name']):
                if 'mongod' in proc.info['name'].lower():
                    try:
                        ...
                        pass
            if killed > 0:
                self.send_json_response({'success': True, 'message': f'Stopped {killed} MongoDB process(es)'})
            else:
