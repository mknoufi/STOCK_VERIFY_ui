...
export interface CachedCountLine extends CreateCountLinePayload {
  _id: string;
  item_name: string;
  counted_by: string;
  counted_at: string;
...
export const updateQueueItemStatus = async (
  ...
  try {
    ...
    __DEV__ && console.error("Error updating queue item status:", error);
  }
};
...
export const getQueueStats = async () => {
  try {
    ...
    return { total: 0, pending: 0, conflicts: 0, byType: {} };
  }
};
