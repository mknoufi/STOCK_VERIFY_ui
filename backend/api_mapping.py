...
class TableMapping(BaseModel):
    ...
    erp_table: str
    db_schema: str = (
        "dbo"  # Renamed from 'schema' to avoid shadowing BaseModel attribute
    )
...
async def get_table_columns(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch columns: {str(e)}"
        )
    finally:
        if conn:
            conn.close()
...
async def get_current_mapping():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch mapping: {str(e)}"
        )
