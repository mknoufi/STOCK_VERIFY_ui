...
## 2026-02-28 - Preserving Focus During Button Loading States
**Learning:** In React Native, conditionally swapping a button's content with an `ActivityIndicator` removes the text element from the accessibility tree, which can cause screen readers to lose focus or fail to announce the "busy" state correctly.
**Action:** Always maintain the original button content but hide it using `opacity: 0` during loading, absolutely position the loading indicator, and set `accessibilityState={{ disabled: true, busy: true }}` to preserve structure and accessibility focus.
