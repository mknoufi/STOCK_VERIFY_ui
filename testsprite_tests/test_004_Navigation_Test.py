...
async def run_test():
    ...
    try:
        ...
        try:
            await expect(page.locator("text=Navigation to Mars Successful").first).to_be_visible(
                timeout=1000
            )
        except AssertionError:
            raise AssertionError(
                "Test case failed: Navigation between different screens did not work as expected, and back button functionality could not be verified."
            )
