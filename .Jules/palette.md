...
## 2026-02-28 - Custom Action Button Accessibility
**Learning:** Custom action buttons built with `TouchableOpacity` (e.g., `Button.tsx`) do not automatically inherit standard screen reader announcements for roles, labels, and states unless explicitly mapped. Omitting these means users are not informed of disabled or loading statuses, or even that the element acts as a button.
**Action:** Always verify that custom touchables explicitly accept and map `accessibilityRole`, `accessibilityLabel`, `accessibilityHint`, and use `accessibilityState` dynamically (e.g., `{{ disabled, busy }}`) for complete accessibility context.
