...
## 2024-03-24 - Accessibility focus loss during button loading states
**Learning:** In React Native, conditionally rendering a loading indicator in place of button text/icons removes the original element from the accessibility tree, breaking screen reader focus and causing disruptive layout jumps.
**Action:** Instead of conditionally replacing content, maintain a single button wrapper, set its `accessibilityState={{ busy: true, disabled: true }}`, hide the original text using `opacity: 0`, and absolute position the `<ActivityIndicator>`.
