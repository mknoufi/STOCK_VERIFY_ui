...
## 2026-03-01 - Checkbox and Radio Selection Accessibility
**Learning:** Interactive selectors like custom `Checkbox` and `Radio` built with `TouchableOpacity` are inaccessible when proper ARIA roles and states are missing. Screen reader users won't know they are interactable selections or what their current state is.
**Action:** Always ensure that `Checkbox` and `Radio` implementations explicitly set `accessibilityRole="checkbox"` and `accessibilityRole="radio"` respectively, while dynamically keeping `accessibilityState` up to date to reflect `checked` and `disabled` values.
