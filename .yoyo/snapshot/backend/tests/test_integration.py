...
class TestCountLineWorkflow:
    ...
    @pytest.mark.asyncio
    async def test_count_line_creation(self, async_client: AsyncClient, auth_headers: Dict[str, str]):
        ...
        response = await async_client.post("/api/count-lines", json=count_line_data, headers=auth_headers)
...
class TestERPItemsWorkflow:
    ...
    @pytest.mark.asyncio
    async def test_erp_items_barcode_lookup(self, async_client: AsyncClient, auth_headers: Dict[str, str]):
        """Test looking up ERP item by barcode"""
        logger.info("Testing ERP items barcode lookup")

        # Test barcode lookup
        test_barcode = "TEST123456"
        response = await async_client.get(f"/api/erp/items/barcode/{test_barcode}", headers=auth_headers)
