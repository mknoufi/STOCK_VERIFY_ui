...
## 2026-07-20 - Button Focus Loss During Loading State
**Learning:** In React Native, returning a completely separate loading component (like an `ActivityIndicator` wrapped in a new `TouchableOpacity`) conditionally inside a button replaces the existing element in the accessibility tree, which drops keyboard/screen reader focus and breaks continuous interaction.
**Action:** Always maintain a single button wrapper (`TouchableOpacity`/`Pressable`) throughout state transitions. To handle loading states, use `accessibilityState={{ busy: true, disabled: true }}`, hide the original content with `opacity: 0`, and overlay the loading indicator using absolute positioning.
