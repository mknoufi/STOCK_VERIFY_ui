...
## 2026-06-27 - Refresh Button Accessibility
**Learning:** Icon-only utility buttons like 'Refresh' are often overlooked and provide no context to screen reader users if missing proper accessibility props.
**Action:** Always add `accessibilityRole="button"` and an explicit `accessibilityLabel` (e.g., 'Refresh') to icon-only utility elements, along with `accessibilityState` for dynamic states like busy and disabled.
