...
\n## 2024-04-20 - Accordion Component Screen Reader Support\n**Learning:** The custom Accordion component built with `TouchableOpacity` headers completely failed to announce its purpose and expanded/collapsed state to screen readers by default.\n**Action:** Always verify custom expandable containers implemented with base touchable elements map `accessibilityRole="button"` and dynamically map their open/closed state to `accessibilityState={{ expanded }}`.
