...
**Learning:** Custom expandable components like `Accordion` sections built with `TouchableOpacity` are difficult for screen reader users to understand if they do not explicitly announce their state.
**Action:** Always include `accessibilityRole="button"`, `accessibilityState={{ expanded: boolean }}`, `accessibilityLabel`, and `accessibilityHint` to properly announce expanded/collapsed state and toggling action to screen readers.
