...
# Helper Functions
def build_date_filter(date_from: Optional[date], date_to: Optional[date]) -> Dict:
            ...
            "$project": {
                ...
                "duration_minutes": {
                    "$cond": {
                        ...
                        "then": {
                            "$divide": [{"$subtract": ["$completed_at", "$started_at"]}, 60000]
                        },
                        "else": None,
                    }
                },
...
async def generate_report(
    request: ReportRequest, current_user: dict = Depends(require_role("admin", "supervisor"))
):
    """
...
async def export_report_csv(
    ...
    if not data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No data found for the specified filters"
        )
...
async def export_report_xlsx(
    ...
    ws = wb.active
    ws.title = REPORT_TYPES[request.report_type]["name"][:31]  # Excel limits sheet name to 31 chars
...
async def get_report_filter_options(
    ...
    try:
        ...
        if current_user.get("role") in ["admin", "supervisor"]:
            ...
            users = [
                {"id": str(u["_id"]), "username": u["username"], "role": u.get("role", "staff")}
