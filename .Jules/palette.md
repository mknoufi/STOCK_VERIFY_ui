...
## 2024-06-29 - Icon-Only RefreshButton Accessibility
**Learning:** Transient state icon buttons like `RefreshButton` are completely silent to screen readers unless fully labeled. Crucially, the "busy" state needs to be conveyed via `accessibilityState` so users know when data is refreshing, not just when it is disabled.
**Action:** Always add `accessibilityRole="button"`, explicit labels/hints, and bind `accessibilityState={{ disabled: loading, busy: loading }}` to interactive components that toggle asynchronous actions.
