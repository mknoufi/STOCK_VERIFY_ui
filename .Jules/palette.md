...
## 2024-06-25 - Button Loading State Focus
**Learning:** Completely replacing a button's content with an `ActivityIndicator` removes the text/icon from the accessibility tree, breaking screen reader focus on the button.
**Action:** When creating loading states for buttons, maintain the original layout (text/icon) but set opacity to 0, and absolutely position the `ActivityIndicator` over it. This keeps the button accessible and prevents layout shifts.
