...
## 2024-06-25 - Accordion/Disclosure Accessibility
**Learning:** Custom expandable components like `Accordion` sections built with `TouchableOpacity` are seen merely as generic clickable elements by screen readers unless explicitly typed. Without `accessibilityState={{ expanded: boolean }}`, users have no idea if clicking it will expand or collapse content.
**Action:** Always include `accessibilityRole="button"`, `accessibilityState={{ expanded: boolean }}`, `accessibilityLabel`, and `accessibilityHint` on custom disclosure widgets to properly announce their state and function.
