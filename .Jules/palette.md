...
## 2026-03-01 - Accordion Accessibility
**Learning:** Expandable/collapsible components like `Accordion` sections built with `TouchableOpacity` must explicitly include accessibility props (like `accessibilityRole="button"`, `accessibilityState={{ expanded: boolean }}`, `accessibilityLabel`, and `accessibilityHint`) to properly announce their state and toggling action to screen readers.
**Action:** Always add these dynamic accessibility attributes to custom interactive components that toggle content visibility.
