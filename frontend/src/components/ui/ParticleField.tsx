...
const ParticleElement: React.FC<{ particle: Particle; color: string; animated: boolean }> = ({
  particle,
  color,
  animated,
}) => {
  ...
  useEffect(() => {
    if (animated) {
      ...
      translateY.value = withDelay(
        ...
        withRepeat(
          withSequence(
            withTiming(-30, { duration: 3000 + Math.random() * 2000, easing: Easing.inOut(Easing.ease) }),
            withTiming(30, { duration: 3000 + Math.random() * 2000, easing: Easing.inOut(Easing.ease) })
          ),
          -1,
          true
      ...
      opacity.value = withDelay(
        ...
        withRepeat(
          withSequence(
            withTiming(particle.opacity * 1.5, { duration: 2000, easing: Easing.inOut(Easing.ease) }),
            withTiming(particle.opacity * 0.5, { duration: 2000, easing: Easing.inOut(Easing.ease) })
          ),
          -1,
          true
        )
