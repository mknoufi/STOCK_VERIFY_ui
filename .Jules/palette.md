...
## 2024-06-25 - Floating Action Menus & Invisible Backdrops
**Learning:** Floating action menus and speed dials often lack `accessibilityRole` and `accessibilityLabel` on both their toggle buttons and action items. Additionally, the invisible backdrop used to dismiss these menus is invisible to screen readers without `accessible={true}`, `accessibilityRole="button"`, and an `accessibilityLabel` (like "Close menu").
**Action:** Always verify floating action menus expose and utilize `accessibilityRole`, `accessibilityState`, and `accessibilityLabel` on triggers and sub-items, and ensure invisible dismiss backdrops are explicitly made accessible.
