    ...
    <>
      ...
      <AuroraBackground variant="secondary" intensity="medium" animated={true}>
        <ScrollView
          ...
          refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={modernColors.primary[500]} />}
        >
          {/* Header */}
            ...
            <GlassCard variant="medium" style={styles.servicesCard}>
              ...
              <View style={styles.serviceList}>
                <ServiceItem
                  ...
                  onToggle={() => handleServiceAction('backend', services.backend?.running ? 'stop' : 'start')}
                />
                <ServiceItem
                  name="frontend"
                  status={services.frontend}
                  loading={actionLoading === 'frontend'}
                  onToggle={() => handleServiceAction('frontend', services.frontend?.running ? 'stop' : 'start')}
                />
                <ServiceItem
                  name="database"
              ...
              {issues.map((issue, index) => (
                <GlassCard key={index} variant="medium" style={styles.issueCard}>
                  ...
                  <View style={styles.issueContent}>
                    ...
                    <Text style={styles.issueService}>{issue.service} • {issue.type}</Text>
                  </View>
                </GlassCard>
              ))}
            </Animated.View>
