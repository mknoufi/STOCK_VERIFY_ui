...
## 2025-02-28 - Preserving Screen Reader Focus during Async Actions
**Learning:** In React Native, conditionally swapping a component (e.g., an icon) for a loading indicator inside a button can remove the element from the accessibility tree, breaking screen reader focus and causing layout shifts.
**Action:** Instead of conditional rendering, keep the original content in the tree with `opacity: 0` when loading, absolutely position the `ActivityIndicator` over it, and use `accessibilityState={{ disabled: loading, busy: loading }}` to communicate the async state.
