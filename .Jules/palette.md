...
## 2026-03-27 - AppHeader Accessibility Props
**Learning:** Icon-only navigation and action buttons (like Notifications, Settings, Logout, and Back) in the main AppHeader component were missing context for screen reader users.
**Action:** Always ensure `accessibilityRole="button"`, `accessibilityLabel`, and `accessibilityHint` are added to `TouchableOpacity` or `Pressable` components when they only contain icons.
