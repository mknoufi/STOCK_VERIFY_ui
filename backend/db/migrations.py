...
class MigrationManager:
    ...
    async def ensure_indexes(self):
        ...
        try:
            ...
            await self._ensure_item_variances_indexes()
            await self._ensure_erp_config_indexes()
            await self._ensure_sync_metadata_indexes()
            await self._ensure_activity_logs_indexes()
    ...
    async def _ensure_users_indexes(self):
    ...
    async def _cleanup_duplicate_users(self):
        ...
        if duplicates:
            ...
            for dup in duplicates:
                ...
                users = await self.db.users.find({"username": dup["_id"]}).to_list(None)
                if len(users) > 1:
                    # Keep the oldest one
    ...
    async def _ensure_refresh_tokens_indexes(self):
    ...
    async def _ensure_sessions_indexes(self):
    ...
    async def _ensure_count_lines_indexes(self):
        ...
        await self.db.count_lines.create_index([("session_id", 1), ("item_code", 1)])
        await self.db.count_lines.create_index([("session_id", 1), ("counted_at", -1)])
        await self.db.count_lines.create_index([("session_id", 1), ("status", 1)])
        await self.db.count_lines.create_index([("item_code", 1), ("counted_at", -1)])
    ...
    async def _ensure_erp_items_indexes(self):
        # ERP items collection indexes (for caching)
        try:
            await self.db.erp_items.create_index("item_code", unique=True)
        except Exception as e:
            if "IndexOptionsConflict" not in str(e) and "already exists" not in str(e):
                logger.warning(f"Error creating erp_items item_code index: {str(e)}")
    ...
    async def _ensure_item_variances_indexes(self):
    ...
    async def _ensure_erp_config_indexes(self):
    ...
    async def _ensure_sync_metadata_indexes(self):
    ...
    async def _ensure_activity_logs_indexes(self):
        # Activity logs collection indexes (recommended from CODEBASE_ANALYSIS.md)
        try:
            await self.db.activity_logs.create_index([("created_at", -1)])  # For time-based queries
