...
## 2024-06-25 - Transient State Accessibility
**Learning:** Transient state icon buttons (like `RefreshButton`) require explicit labeling to be audible to screen readers. Crucially, their 'busy' state must be conveyed via `accessibilityState={{ disabled: loading, busy: loading }}` so users know when an asynchronous action (like data refreshing) is actively occurring.
**Action:** Always add `accessibilityState={{ busy: loading }}` and `disabled: loading` when appropriate to any button triggering an async task.
