...
## 2024-06-15 - Transient Async Buttons Accessibility
**Learning:** Transient state icon buttons (like `RefreshButton`) are completely silent to screen readers unless fully labeled. Crucially, their 'busy' state must be conveyed via `accessibilityState={{ disabled: loading, busy: loading }}` so screen reader users know when an asynchronous action (like data refreshing) is actively occurring and the app hasn't frozen.
**Action:** Always add `accessibilityRole="button"`, `accessibilityLabel`, and `accessibilityState={{ disabled: loading, busy: loading }}` to icon-only refresh or async action buttons.
