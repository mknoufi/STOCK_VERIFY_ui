...
## 2024-05-28 - Custom Accordion Accessibility
**Learning:** Custom expandable components like `Accordion` headers built with `TouchableOpacity` lack default behaviors that inform screen readers of their interactive state. Without explicitly conveying that it's a button and whether it's expanded or collapsed, users are left guessing its purpose and current state.
**Action:** Always include `accessibilityRole="button"`, `accessibilityState={{ expanded: boolean }}`, and a dynamic `accessibilityHint` on custom expandable toggle headers.
