...
## 2026-04-16 - Refresh and Async Icon Button Accessibility
**Learning:** Icon-only action buttons like Refresh often miss contextual accessibility features, and their loading/busy state is often hidden from screen reader users because the visual spinner replaces the icon without updating accessible properties.
**Action:** Always add `accessibilityRole="button"`, default `accessibilityLabel` and `accessibilityHint` to custom action buttons, and pass `accessibilityState={{ disabled: loading, busy: loading }}` to inform screen readers of async operations.
