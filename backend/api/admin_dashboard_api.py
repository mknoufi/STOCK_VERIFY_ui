...
async def get_active_users(current_user: dict = Depends(require_admin)):
    ...
    try:
        ...
        for record in presence_records:
            ...
            if user:
                ...
                session = await db.verification_sessions.find_one(
                    {"user_id": str(user["_id"]), "status": {"$in": ["active", "in_progress"]}}
                )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to fetch active users"
        )
...
async def get_error_logs(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to fetch error logs"
        )
