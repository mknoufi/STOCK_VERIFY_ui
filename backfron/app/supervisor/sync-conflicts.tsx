...
export default function SyncConflictsScreen() {
  ...
  const loadConflicts = async () => {
    try {
      ...
      // 1. Fetch Server Conflicts
      let serverConflicts: SyncConflict[] = [];
      try {
        const response = await getSyncConflicts(status);
        serverConflicts = response.data?.conflicts || [];
      } catch (err) {
        console.warn("Failed to load server conflicts", err);
      }

      // 2. Fetch Local Offline Conflicts
      let localConflicts: SyncConflict[] = [];
      if (filterStatus === "pending" || filterStatus === "all") {
        const offlineQueue = await getOfflineQueue();
        localConflicts = offlineQueue
          .filter((item) => item.status === "conflict")
          .map((item) => ({
            _id: item.id,
            offlineId: item.id,
            session_id: item.data?.sessionId || "unknown",
            item_code:
              item.data?.itemCode || item.data?.barcode || "Unknown Item",
            conflict_type: "Local Sync Error",
            local_value: item.data?.quantity || 0,
            server_value: "Rejected",
            status: "pending", // Mapped to Pending for UI
            detected_at: new Date(item.timestamp).toISOString(),
            isLocal: true,
            error_message: item.error_message,
          }));
      }

      setConflicts([...localConflicts, ...serverConflicts]);
    } catch (error: any) {
  ...
  const handleLocalAction = async (
    ...
    try {
      if (action === "discard") {
        await removeFromOfflineQueue(offlineId);
        Alert.alert("Discarded", "Local conflict item removed.");
      } else if (action === "retry") {
        await updateQueueItemStatus(offlineId, "pending");
      ...
      >
        ...
        <View style={styles.conflictData}>
          <View style={styles.dataColumn}>
            <Text style={styles.dataLabel}>Local Qty:</Text>
            <Text style={styles.dataValue}>{conflict.local_value}</Text>
          </View>
          <View style={styles.dataColumn}>
            <Text style={styles.dataLabel}>Server:</Text>
            <Text style={styles.dataValue}>{conflict.server_value}</Text>
          </View>
        </View>
          ...
          {conflict.isLocal ? (
            <>
              <TouchableOpacity
                ...
                onPress={() => handleLocalAction(conflict.offlineId!, "retry")}
              >
              ...
              <TouchableOpacity
                ...
                onPress={() =>
                  handleLocalAction(conflict.offlineId!, "discard")
                }
              >
        ...
        {conflict.status !== "pending" && !conflict.isLocal && (
