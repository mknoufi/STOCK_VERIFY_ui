...
## 2024-07-30 - Layout Stability & Focus during Loading States
**Learning:** Returning a completely separate loading component inside a button removes it from the accessibility tree and breaks screen reader focus. Additionally, it can cause the button's dimensions to shift.
**Action:** Maintain a single button wrapper, use `accessibilityState={{ busy: true, disabled: true }}`, keep the original text/icon with `opacity: 0`, and position the `ActivityIndicator` absolutely over the hidden content.
