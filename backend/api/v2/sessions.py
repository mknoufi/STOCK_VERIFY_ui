...
async def get_rack_progress(
    ...
    try:
        ...
        if not warehouse:
            return ApiResponse.error_response(
                error_code="INVALID_SESSION", error_message="Session has no warehouse assigned"
            )
...
async def get_watchtower_stats(
    ...
    try:
        ...
        # This requires joining sessions -> items/count_lines which is complex.
        # Simplified: Get recent count lines where we might infer variance or just get recent activity.
        # Ideally, we query a 'variances' collection if it exists, or just return empty for MVP.
        # Let's return recent critical actions for now.
        recent_activity = (
