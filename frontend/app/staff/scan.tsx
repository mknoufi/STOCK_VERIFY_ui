...
export default function ScanScreen() {
  ...
  const [damageType, setDamageType] = useState<"returned" | "returnable" | "nonreturnable">("returnable");
  ...
  const [photoType, setPhotoType] = useState<"item" | "verification" | "serial">("item");
  ...
  useEffect(() => {
    ...
    const load = async () => {
      if (!sessionId || typeof sessionId !== "string") return;
      const list = await RecentRacksService.getRecent(sessionId, itemState.floorNo, 5);
  ...
  const handleLookup = async (barcode: string) => {
    ...
    try {
      ...
      addDebug(`getItemByBarcode returned: ${item ? getItemDisplayName(item) : "null"}`);

      if (item) {
        addDebug(`Item found: ${item.item_code} - ${getItemDisplayName(item)}`);

        // Track analytics
        AnalyticsService.trackItemScan(item.item_code, getItemDisplayName(item)).catch(() => {});
        ...
        if (sessionId && item.item_code) {
          try {
            ...
            if (countCheck.already_counted && countCheck.count_lines?.length > 0) {
  ...
  const prepareItemForCounting = useCallback((item: any) => {
  ...
  const handleSubmitCount = useCallback(async () => {
    ...
    const submitVerifiedCount = async () => {
      ...
      try {
        ...
        if (typeof sessionId === "string" && (itemState.floorNo || "") && (itemState.rackNo || "")) {
          RecentRacksService.addRecent(sessionId, itemState.floorNo, itemState.rackNo!).catch(() => {});
        }

        // Track analytics
        AnalyticsService.trackCount(scannedItem.item_code, Number(quantity)).catch(() => {});
    ...
    scannedItem, quantity, isDamageEnabled, damageQty, isSerialEnabled,
      serialNumbers, sessionId, mrp, remark, damageType,
    damageIncluded, itemPhoto, verificationPhoto, itemState,
    sessionDetails, varianceReasons, resetForm, category, condition, mfgDate, subCategory
  ]);

  // Handle variance reason selection
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const handleVarianceReasonSubmit = useCallback((reason: string, note?: string) => {
    updateItemState({
    ...
    >
          ...
          <View style={styles.scanSection}>
            ...
            {!scannedItem ? (
              ...
              <View style={styles.formSection}>
                    ...
                    <View style={styles.itemImageContainer}>
                      {scannedItem.image_url ? (
                        ...
                        <View style={styles.itemImagePlaceholder}>
                          <Ionicons name="cube" size={40} color={modernColors.primary[400]} />
                        </View>
                      )}
                      {existingCountLine && (
                    ...
                    <View style={styles.itemDetails}>
                      ...
                      <View style={styles.itemMetaRow}>
                        <View style={styles.itemMetaChip}>
                          <Ionicons name="barcode-outline" size={14} color={modernColors.primary[500]} />
                          <Text style={styles.itemMetaText}>{scannedItem.barcode || scannedItem.item_code}</Text>
                        </View>
                        <View style={styles.itemMetaChip}>
                          <Ionicons name="layers-outline" size={14} color={modernColors.text.tertiary} />
                          <Text style={styles.itemMetaText}>{scannedItem.uom || scannedItem.uom_name || "Unit"}</Text>
                        </View>
                      </View>
                      ...
                      <View style={styles.priceRow}>
                        <View style={styles.priceBox}>
                          ...
                          <Text style={styles.priceValue}>₹{scannedItem.mrp || 0}</Text>
                        </View>
                        <View style={styles.priceDivider} />
                        <View style={styles.priceBox}>
                          <Text style={styles.priceLabel}>SP</Text>
                          <Text style={styles.priceValue}>₹{scannedItem.standard_rate || 0}</Text>
                        </View>
                        <View style={styles.priceDivider} />
                        <View style={styles.priceBox}>
                          <Text style={styles.priceLabel}>Stock</Text>
                          <Text style={[styles.priceValue, { color: modernColors.primary[600] }]}>{scannedItem.stock_qty || 0}</Text>
                        </View>
                      </View>

                      <View style={styles.categoryRow}>
                        <Ionicons name="folder-outline" size={14} color={modernColors.text.tertiary} />
                        <Text style={styles.categoryText}>
                          {scannedItem.category || "Uncategorized"} {scannedItem.subcategory ? `• ${scannedItem.subcategory}` : ""}
                        </Text>
                      </View>
                    </View>
                  </View>
                ...
                <View style={styles.stepCard}>
                  <View style={styles.stepHeader}>
                    <View style={[styles.stepBadge, { backgroundColor: modernColors.success.main }]}>
                  ...
                  <View style={styles.quantitySection}>
                    <View style={styles.quantityInputContainer}>
                      ...
                      >
                        <Ionicons name="remove" size={28} color={modernColors.text.primary} />
                      </TouchableOpacity>
                      ...
                      <TouchableOpacity
                        style={[styles.quantityButton, { backgroundColor: modernColors.primary[500] }]}
                      ...
                      {[1, 5, 10, 25, 50, 100].map((num) => (
                        <TouchableOpacity
                          ...
                          style={[
                            ...
                            quantity === String(num) && styles.quickQuantityChipActive
                          ]}
                          onPress={() => setQuantity(String(num))}
                        >
                          <Text style={[
                            styles.quickQuantityText,
                            quantity === String(num) && styles.quickQuantityTextActive
                          ]}>{num}</Text>
                        </TouchableOpacity>
                      ))}
                    </View>
                ...
                <View style={styles.stepCard}>
                  <View style={styles.stepHeader}>
                    <View style={[styles.stepBadge, { backgroundColor: modernColors.warning.main }]}>
                    ...
                    >
                      <Text style={styles.selectorText}>{condition || "Good"}</Text>
                      <Ionicons name="chevron-down" size={16} color={modernColors.text.tertiary} />
                    </TouchableOpacity>
                  </View>
                  ...
                  <View style={styles.conditionRow}>
                    <TouchableOpacity
                      ...
                      onPress={() => { setCondition('Good'); setIsDamageEnabled(false); }}
                    ...
                    >
                      <Ionicons
                        ...
                        size={32}
                        color={condition === 'Good' ? modernColors.success.main : modernColors.text.tertiary}
                      />
                      <Text style={[
                        styles.conditionText,
                        condition === 'Good' && { color: modernColors.success.main, fontWeight: '700' }
                      ]}>Good</Text>
                    </TouchableOpacity>
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() => { setCondition('Damage'); setIsDamageEnabled(true); }}
                    ...
                    >
                      <Ionicons
                        name="alert-circle"
                        size={32}
                        color={condition === 'Damage' ? modernColors.error.main : modernColors.text.tertiary}
                      />
                      <Text style={[
                        styles.conditionText,
                        condition === 'Damage' && { color: modernColors.error.main, fontWeight: '700' }
                      ]}>Damaged</Text>
                    </TouchableOpacity>
                  </View>
                  ...
                  {isDamageEnabled && (
                    <View style={styles.damageDetails}>
                      <View style={styles.damageTypeSelector}>
                        ...
                        >
                          <Ionicons
                            name={damageType === 'returnable' ? "radio-button-on" : "radio-button-off"}
                            size={20}
                            color={damageType === 'returnable' ? modernColors.error.main : modernColors.text.tertiary}
                          />
                          <Text style={styles.damageTypeText}>Returnable</Text>
                        </TouchableOpacity>
                        <TouchableOpacity
                          style={[
                            styles.damageTypeOption,
                        ...
                        >
                          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, marginTop: 12 }}>
                            ...
                            <TextInput
                              style={[styles.quantityInput, { flex: 0, minWidth: 100 }]}
                              value={damageQty}
                              onChangeText={(text) => {
                                const sanitized = text.replace(/[^0-9.]/g, "");
                                const parts = sanitized.split(".");
                          ...
                          <Ionicons
                            name={damageType === 'nonreturnable' ? "radio-button-on" : "radio-button-off"}
                            size={20}
                            color={damageType === 'nonreturnable' ? modernColors.error.main : modernColors.text.tertiary}
                          />
                          <Text style={styles.damageTypeText}>Non-Returnable</Text>
                        </TouchableOpacity>
                      </View>
                      <TextInput
                ...
                >
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                    <View style={[styles.stepBadge, { backgroundColor: modernColors.text.tertiary }]}>
                  ...
                  <View style={styles.additionalDetailsCard}>
                    ...
                    <View style={styles.photoUploadSection}>
                      ...
                      <TouchableOpacity
                        ...
                        onPress={() => { setPhotoType("item"); setShowPhotoModal(true); }}
                        accessibilityLabel="Add item photo"
                        testID="btn-add-item-photo"
                      >
                        {itemPhoto ? (
                          <View style={styles.photoPreviewContainer}>
                            <Image source={{ uri: itemPhoto.uri }} style={styles.photoPreviewSmall} />
                          ...
                          <>
                            <Ionicons name="camera-outline" size={24} color={modernColors.primary[500]} />
                            <Text style={styles.photoUploadText}>Add Photo (for later reference)</Text>
                          </>
                        )}
                      </TouchableOpacity>
                      <Text style={{ color: modernColors.text.tertiary, fontSize: 12, marginTop: 6 }}>
                    ...
                    <View style={[
                      ...
                      <View style={styles.serialToggleHeader}>
                        <Ionicons
                          name="qr-code"
                          size={20}
                          color={isSerialEnabled ? modernColors.primary[500] : modernColors.text.secondary}
                        />
                        <View style={{ flex: 1 }}>
                          <Text style={styles.serialToggleTitle}>Track Serial Numbers</Text>
                          <Text style={styles.serialToggleSubtitle}>Scan individual serials</Text>
                        </View>
                        <Switch
                            ...
                            <View style={styles.serialList}>
                              {serialNumbers.map((sn, index) => (
                                <View key={index} style={styles.serialChip}>
                                  ...
                                  <TouchableOpacity
                                    onPress={() => setSerialNumbers(serialNumbers.filter((s) => s !== sn))}
                                    accessibilityLabel={`Remove serial ${sn}`}
                                    testID={`btn-remove-serial-${index}`}
                                  >
                                    <Ionicons name="close-circle" size={16} color={modernColors.text.tertiary} />
                                  </TouchableOpacity>
                                </View>
                              ))}
                            </View>
                ...
                <View style={styles.submitSection}>
                  <TouchableOpacity style={styles.cancelButton} onPress={resetForm} accessibilityLabel="Cancel" testID="btn-cancel">
                    <Text style={styles.cancelButtonText}>Cancel</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={[
                      styles.submitButton,
                      (!scannedItem || !quantity || Number(quantity) <= 0) && styles.submitButtonDisabled
                    ]}
                    onPress={() => setShowVerificationModal(true)}
                    disabled={!scannedItem || !quantity || Number(quantity) <= 0 || loading}
            ...
            <TouchableOpacity activeOpacity={1} onPress={(e) => e.stopPropagation()}>
              <View style={styles.modalContent}>
                <View style={styles.modalHeader}>
                  <Ionicons name="add-circle-outline" size={48} color={modernColors.primary[500]} />
                  <Text style={styles.modalTitle}>Add Quantity</Text>
                </View>

                {scannedItem && existingCountLine && (
                  <View style={styles.addQtyInfo}>
                    <Text style={styles.addQtyItemName}>{getItemDisplayName(scannedItem)}</Text>
                    <Text style={styles.addQtyItemCode}>{scannedItem.item_code}</Text>

                    <View style={styles.addQtyCurrentContainer}>
                      <Text style={styles.addQtyLabel}>Current Count:</Text>
                      <Text style={styles.addQtyValue}>
                        {existingCountLine.counted_qty || 0} {scannedItem.uom_name || ""}
                      </Text>
                    </View>
                    ...
                    {additionalQty && !isNaN(parseFloat(additionalQty)) && (
                      <View style={styles.addQtyNewTotal}>
                        ...
                        <Text style={styles.addQtyTotalValue}>
                          {((existingCountLine.counted_qty || 0) + parseFloat(additionalQty)).toFixed(2)} {scannedItem.uom_name || ""}
                        </Text>
                      </View>
                    )}
                  </View>
                ...
                <View style={styles.modalButtons}>
                  ...
                  <PremiumButton
                    ...
                    loading={loading}
                    disabled={!additionalQty || isNaN(parseFloat(additionalQty)) || parseFloat(additionalQty) <= 0}
                    style={{ flex: 1 }}
                  />
                </View>
              </View>
            ...
            <ScrollView style={{ maxHeight: 300 }}>
              {varianceReasons.map((reason) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.reasonOption,
                    itemState.selectedReason === reason.code && styles.reasonSelected,
                  ]}
                  onPress={() => updateItemState({ selectedReason: reason.code })}
                >
                  <Text style={styles.reasonText}>{reason.label || reason.description}</Text>
                </TouchableOpacity>
              ))}
            </ScrollView>
      ...
      <Modal visible={showVerificationModal} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Ionicons name="shield-checkmark" size={48} color={modernColors.primary[500]} />
              <Text style={styles.modalTitle}>Verify Details</Text>
              <Text style={styles.modalSubtitle}>Please confirm the item details before submitting</Text>
            </View>
            ...
            <ScrollView style={{ maxHeight: 400 }}>
                <View style={{ gap: 16 }}>
                    <View style={styles.verificationRow}>
                        ...
                        <Text style={[styles.verificationValue, { flex: 1, textAlign: 'right' }]}>{getItemDisplayName(scannedItem)}</Text>
                    </View>

                    <View style={styles.verificationRow}>
                        <Text style={styles.verificationLabel}>Quantity</Text>
                        <Text style={styles.verificationValue}>{quantity} {scannedItem?.uom || scannedItem?.uom_name}</Text>
                    </View>

                    <View style={styles.verificationRow}>
                        <Text style={styles.verificationLabel}>Condition</Text>
                        <Text style={[styles.verificationValue, { color: condition === 'Damage' ? modernColors.error.main : modernColors.success.main }]}>{condition || 'Good'}</Text>
                    </View>

                    {isDamageEnabled && (
                        <View style={styles.damageReportBox}>
                            <Text style={{ color: modernColors.error.main, fontWeight: '600', marginBottom: 4 }}>Damage Report</Text>
                            <Text style={{ color: modernColors.text.primary }}>Type: {damageType === 'nonreturnable' ? 'Non-Returnable' : 'Returnable'}</Text>
                            {damageRemark ? <Text style={{ color: modernColors.text.secondary, marginTop: 4 }}>&quot;{damageRemark}&quot;</Text> : null}
                        </View>
                    )}

                    {isSerialEnabled && (
                        <View style={styles.serialReportBox}>
                            <Text style={{ color: modernColors.primary[700], fontWeight: '600', marginBottom: 4 }}>Serial Numbers ({serialNumbers.length})</Text>
                            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 4 }}>
                                {serialNumbers.map(s => (
                                    <Text key={s} style={styles.serialTag}>{s}</Text>
                                ))}
                            </View>
                        </View>
                    )}
