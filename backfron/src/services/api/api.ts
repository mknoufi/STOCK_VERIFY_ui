...
import { CreateCountLinePayload } from "../../types";
 ...
 * @param sessionId Optional session ID to check for duplicate scans
 */
export const getItemByBarcode = async (
  ...
  try {
    ...
    const response = await retryWithBackoff(
      () =>
        api.get(
          `/api/v2/erp/items/barcode/${encodeURIComponent(trimmedBarcode)}/enhanced${queryParams}`,
        ),
      {
        retries: retryCount,
        backoffMs: 1000,
    ...
    // Attach previous count info if available
    if (response.data.metadata?.previous_count) {
      itemData.previous_count = response.data.metadata.previous_count;
    }
    ...
    try {
      ...
      if (apiError.response?.status === 404) {
        ...
        throw new Error(
          `Barcode lookup failed: ${apiError.message || "Unknown error"}`,
        );
      }
    }
  }
...
export const searchItems = async (query: string) => {
  try {
    ...
    const mappedItems = items.map((item: any) => {
      const mapped = { ...item };
      if (mapped.item_name && !mapped.name) {
        mapped.name = mapped.item_name;
      }
...
export const createCountLine = async (
  ...
  meta: { itemName: string; username: string },
  skipOfflineFallback = false,
) => {
  try {
    if (!isOnline()) {
    ...
    // Ensure cached data has required display fields
    const countLineToCache = {
      ...response.data,
      item_name: meta.itemName,
      counted_by: meta.username,
      // Ensure specific fields if backend response varies
      item_code: countData.item_code,
      session_id: countData.session_id,
      counted_qty: countData.counted_qty,
    };

    await cacheCountLine(countLineToCache);
    return response.data;
  } catch (error) {
    ...
    if (!isOnline()) {
      ...
      if (verified !== undefined) {
        const filtered = cachedLines.filter(
          (line: any) => line.verified === verified,
        );
        return {
          items: filtered,
          pagination: {
...
export const checkItemCounted = async (sessionId: string, itemCode: string) => {
  try {
    ...
    const response = await api.get(
      `/count-lines/check/${sessionId}/${itemCode}`,
    );
    return response.data;
  } catch (error) {
...
export const refreshItemStock = async (itemCode: string) => {
  try {
    const response = await api.post(
      `/erp/items/${encodeURIComponent(itemCode)}/refresh-stock`,
    );
    return response.data;
  } catch (error: any) {
...
export const addUserPermissions = async (
  ...
  try {
    const response = await api.post(`/permissions/users/${username}/add`, {
      permissions,
    });
    return response.data;
  } catch (error: any) {
...
export const removeUserPermissions = async (
  ...
  try {
    const response = await api.post(`/permissions/users/${username}/remove`, {
      permissions,
    });
    return response.data;
  } catch (error: any) {
...
export const getServiceLogs = async (
  ...
  try {
    ...
    const response = await api.get(
      `/admin/control/logs/${service}?${params.toString()}`,
    );
    return response.data;
  } catch (error: any) {
...
export const getFailedLogins = async (
  ...
  try {
    ...
    const response = await api.get(
      `/admin/security/failed-logins?${params.toString()}`,
    );
    return response.data;
  } catch (error: any) {
...
export const getSuspiciousActivity = async (hours: number = 24) => {
  try {
    const response = await api.get(
      `/admin/security/suspicious-activity?hours=${hours}`,
    );
    return response.data;
  } catch (error: any) {
...
export const getSecurityAuditLog = async (
  ...
  try {
    ...
    if (user) params.append("user", user);
    const response = await api.get(
      `/admin/security/audit-log?${params.toString()}`,
    );
    return response.data;
  } catch (error: any) {
...
export const getIpTracking = async (hours: number = 24) => {
  try {
    const response = await api.get(
      `/admin/security/ip-tracking?hours=${hours}`,
    );
    return response.data;
  } catch (error: any) {
...
export const clearServiceLogs = async (service: string) => {
  try {
    const response = await api.post("/admin/control/logs/clear", null, {
      params: { service },
    });
    return response.data;
  } catch (error: any) {
...
export const getFieldDefinitions = async (
  ...
  try {
    ...
    const response = await api.get(
      `/dynamic-fields/definitions?${params.toString()}`,
    );
    return response.data;
  } catch (error: any) {
...
export const createFieldDefinition = async (fieldData: any) => {
  try {
    const response = await api.post("/dynamic-fields/definitions", fieldData);
    return response.data;
  } catch (error: any) {
...
export const updateFieldDefinition = async (fieldId: string, updates: any) => {
  try {
    const response = await api.put(
      `/dynamic-fields/definitions/${fieldId}`,
      updates,
    );
    return response.data;
  } catch (error: any) {
...
export const deleteFieldDefinition = async (fieldId: string) => {
  try {
    const response = await api.delete(`/dynamic-fields/definitions/${fieldId}`);
    return response.data;
  } catch (error: any) {
...
export const setFieldValue = async (
  ...
  try {
    const response = await api.post("/dynamic-fields/values", {
      item_code: itemCode,
...
export const setBulkFieldValues = async (
  ...
  try {
    const response = await api.post("/dynamic-fields/values/bulk", {
...
export const getItemFieldValues = async (itemCode: string) => {
  try {
    const response = await api.get(`/dynamic-fields/values/${itemCode}`);
    return response.data;
  } catch (error: any) {
...
export const getItemsWithFields = async (
  ...
  try {
    ...
    const response = await api.get(
      `/dynamic-fields/items?${params.toString()}`,
    );
    return response.data;
  } catch (error: any) {
...
export const getFieldStatistics = async (fieldName: string) => {
  try {
    const response = await api.get(`/dynamic-fields/statistics/${fieldName}`);
    return response.data;
  } catch (error: any) {
