...
## 2024-05-24 - AppHeader Icon-Only Action Buttons Accessibility
**Learning:** App headers often contain globally relevant icon-only buttons (like Notifications, Settings, Logout) that are skipped or announced poorly by screen readers if accessibility attributes are missing.
**Action:** Always add `accessibilityRole="button"`, an explicit `accessibilityLabel` (e.g., "Notifications", "Settings"), and a descriptive `accessibilityHint` to `Pressable` and `TouchableOpacity` elements used in navigation headers.
