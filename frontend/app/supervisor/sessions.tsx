...
export default function SessionsList() {
  ...
  const loadSessions = useCallback(async (pageNum: number, shouldRefresh = false) => {
    try {
  ...
  const renderItem = ({ item, index }: { item: any, index: number }) => {
    ...
    return (
      <Animated.View entering={FadeInDown.delay(index * 50).springify()}>
        ...
        >
          <GlassCard variant="medium" padding={auroraTheme.spacing.md} borderRadius={auroraTheme.borderRadius.lg} elevation="sm">
            <View style={styles.cardHeader}>
              <View style={styles.cardHeaderLeft}>
                <View style={styles.titleRow}>
                  <Ionicons name="business-outline" size={20} color={auroraTheme.colors.primary[500]} />
                  <Text style={styles.warehouseName}>{item.warehouse}</Text>
                </View>
                <View style={styles.staffContainer}>
                  <Ionicons name="person-outline" size={14} color={auroraTheme.colors.text.secondary} />
            ...
            <View style={styles.cardBody}>
              ...
              <View style={styles.statItem}>
                ...
                <Text
                  style={[
                    styles.statText,
                    hasVariance && { color: auroraTheme.colors.error[500], fontWeight: '600' },
                  ]}
                >
  ...
  const renderEmpty = () => (
    <View style={styles.emptyContainer}>
      <GlassCard variant="medium" style={styles.emptyCard} padding={auroraTheme.spacing.xl}>
        <Ionicons
          name="cube-outline"
          size={64}
