...
const METRIC_CONFIG: Record<MetricType, { label: string; unit: string; color: string; icon: keyof typeof MaterialCommunityIcons.glyphMap }> = {
  response_time: {
    label: "Response Time",
    unit: "ms",
...
function StatCard({ value, unit, label, icon, color }: { value: number | string; unit: string; label: string; icon: keyof typeof MaterialCommunityIcons.glyphMap; color: string }) {
  return (
    ...
    <View style={styles.container}>
      ...
      <View style={styles.statsRow}>
        ...
        <StatCard
          ...
          label="Errors"
          icon="alert-circle-outline"
          color={stats.totalErrors > 0 ? METRIC_CONFIG.errors.color : auroraTheme.colors.success[500]}
        />
      </View>
        ...
        {tabs.map((tab) => (
          ...
          >
            <MaterialCommunityIcons
              ...
              size={16}
              color={activeTab === tab ? auroraTheme.colors.primary[500] : auroraTheme.colors.text.secondary}
            />
            <Text style={[
              styles.tabText,
