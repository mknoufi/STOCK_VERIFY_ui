...
## 2024-07-08 - Maintaining Screen Reader Focus During Loading States
**Learning:** In React Native, conditionally rendering a completely separate loading component inside a button removes the original button from the accessibility tree. This breaks screen reader focus, leaving users confused about the current state.
**Action:** To maintain focus, keep the original button wrapper and its accessibility properties. Instead of replacing the content, use `accessibilityState={{ busy: true, disabled: true }}`, hide the original text/icon with `opacity: 0`, and absolutely position the `ActivityIndicator` over it.
