...
## 2026-03-01 - Search Component Accessibility
**Learning:** The main TextInput used for search requires `accessibilityRole="search"` alongside a helpful label, and any dynamically rendered icon-only clear buttons require full accessibility mapping (role, label, hint) as they would otherwise be completely hidden to screen readers.
**Action:** Always add `accessibilityRole="search"` to search inputs and verify internal icon buttons have explicit labels and hints.
