...
## 2026-06-17 - Accordion Header Accessibility
**Learning:** Custom expandable components like `Accordion` sections built with `TouchableOpacity` must explicitly include `accessibilityRole="button"`, `accessibilityState={{ expanded: boolean }}`, `accessibilityLabel`, and `accessibilityHint` to properly announce their expanded/collapsed state and toggling action to screen readers.
**Action:** Always ensure expandable list/accordion items announce their dynamic state (expanded vs collapsed) and provide context to screen reader users about what activating the item will do.
