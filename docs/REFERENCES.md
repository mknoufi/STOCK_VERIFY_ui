# Official References (All-in-One)
## Stock Verify System

This file lists the **authoritative** external references for this repo.

Blogs, tutorials, or unofficial sources are NOT considered valid references.

---

## Quick Open (matches VS Code tasks)
- FastAPI: https://fastapi.tiangolo.com/
- MongoDB Manual: https://www.mongodb.com/docs/manual/
- React Native: https://reactnative.dev/docs/getting-started
- Expo: https://docs.expo.dev/
- Redis: https://redis.io/docs/

---

## Backend – Python & FastAPI
- Python 3.11
	- Docs: https://docs.python.org/3.11/
	- Source: https://github.com/python/cpython

- FastAPI
	- Docs: https://fastapi.tiangolo.com/
	- GitHub: https://github.com/tiangolo/fastapi

- Uvicorn
	- Docs: https://www.uvicorn.org/
	- GitHub: https://github.com/encode/uvicorn

- Gunicorn
	- Docs: https://docs.gunicorn.org/
	- GitHub: https://github.com/benoitc/gunicorn

---

## Databases & Data Access
- MongoDB (Server)
	- Docs: https://www.mongodb.com/docs/manual/
	- GitHub: https://github.com/mongodb/mongo

- Motor (Async MongoDB Driver – Preferred)
	- Docs: https://motor.readthedocs.io/
	- GitHub: https://github.com/mongodb/motor

- PyMongo (Sync – Limited Use)
	- Docs: https://pymongo.readthedocs.io/
	- GitHub: https://github.com/mongodb/mongo-python-driver

- SQL Server
	- Docs: https://learn.microsoft.com/en-us/sql/sql-server/
	- ODBC Docs: https://learn.microsoft.com/en-us/sql/connect/odbc/

- pyODBC
	- Docs: https://github.com/mkleehammer/pyodbc/wiki
	- GitHub: https://github.com/mkleehammer/pyodbc

- Redis
	- Docs: https://redis.io/docs/
	- GitHub: https://github.com/redis/redis

- redis-py
	- Docs: https://redis-py.readthedocs.io/
	- GitHub: https://github.com/redis/redis-py

---

## Security & Auth
- Pydantic v2
	- Docs: https://docs.pydantic.dev/latest/
	- GitHub: https://github.com/pydantic/pydantic

- PyJWT
	- Docs: https://pyjwt.readthedocs.io/
	- GitHub: https://github.com/jpadilla/pyjwt

- Authlib
	- Docs: https://docs.authlib.org/
	- GitHub: https://github.com/lepture/authlib

- Argon2 (argon2-cffi)
	- Docs: https://argon2-cffi.readthedocs.io/
	- GitHub: https://github.com/hynek/argon2-cffi

---

## Frontend – React Native & Expo
- React
	- Docs: https://react.dev/
	- GitHub: https://github.com/facebook/react

- React Native
	- Docs: https://reactnative.dev/docs/getting-started
	- GitHub: https://github.com/facebook/react-native

- Expo (SDK 54)
	- Docs: https://docs.expo.dev/
	- GitHub: https://github.com/expo/expo

- Expo Router
	- Docs: https://expo.github.io/router/docs
	- GitHub: https://github.com/expo/router

- FlashList
	- Docs: https://shopify.github.io/flash-list/
	- GitHub: https://github.com/Shopify/flash-list

- Zustand
	- Docs: https://docs.pmnd.rs/zustand
	- GitHub: https://github.com/pmndrs/zustand

- TanStack React Query
	- Docs: https://tanstack.com/query/latest
	- GitHub: https://github.com/TanStack/query

- Axios
	- Docs: https://axios-http.com/docs/intro
	- GitHub: https://github.com/axios/axios

- React Native MMKV
	- Docs: https://github.com/mrousavy/react-native-mmkv
	- GitHub: https://github.com/mrousavy/react-native-mmkv

- Async Storage
	- Docs: https://react-native-async-storage.github.io/async-storage/
	- GitHub: https://github.com/react-native-async-storage/async-storage

---

## Infrastructure & Tooling
- Docker
	- Docs: https://docs.docker.com/
	- GitHub: https://github.com/docker

- Docker Compose
	- Docs: https://docs.docker.com/compose/
	- Spec: https://github.com/compose-spec/compose-spec

---

## Source Control & IDE
- Git
	- Docs: https://git-scm.com/doc
	- GitHub: https://github.com/git/git

- GitHub Copilot
	- Docs: https://docs.github.com/en/copilot
	- GitHub: https://github.com/features/copilot

---

## GOVERNANCE RULE

Only the documentation and repositories listed in this file are considered
authoritative for the Stock Verify System.

Any deviation must be justified with official documentation.
